    ...
    public static class TaskHelpers
    {
        ...
        public static async Task ExecuteJob(TaskSettings taskSettings, HotkeyType job, string filePath = null)
        {
            ...
            switch (job)
            {
                ...
                case HotkeyType.SmartSubjectCrop:
                    {
                        Image rawImg = null;
                        using (RegionCaptureForm cap = new RegionCaptureForm(RegionCaptureMode.Default, new RegionCaptureOptions()))
                        {
                            if (cap.ShowDialog() == DialogResult.OK)
                                rawImg = cap.GetResultImage();
                        }
                        if (rawImg != null)
                        {
                            Bitmap cropped = SmartCaptureManager.SmartCropSubject(new Bitmap(rawImg));
                            rawImg.Dispose();
                            UploadManager.RunImageTask(cropped, safeTaskSettings);
                        }
                    }
                    break;

                case HotkeyType.ScreenDiff:
                    {
                        Bitmap diffResult = SmartCaptureManager.CaptureAndDiff(safeTaskSettings);
                        if (diffResult != null)
                            UploadManager.RunImageTask(diffResult, safeTaskSettings);
                    }
                    break;

                case HotkeyType.ClipboardHistory:
                    SmartCaptureManager.ShowClipboardHistory();
                    break;

                case HotkeyType.StartTimelapse:
                    SmartCaptureManager.StartTimelapse(
                        Program.Settings.TimelapseIntervalSeconds > 0
                            ? Program.Settings.TimelapseIntervalSeconds
                            : 5,
                        safeTaskSettings);
                    break;

                case HotkeyType.StopTimelapse:
                    SmartCaptureManager.StopTimelapse();
                    break;

                case HotkeyType.CapturePrivacyBlur:
                    await SmartCaptureManager.CaptureWithPrivacyBlur(safeTaskSettings);
                    break;

                case HotkeyType.SmartAnnotate:
                    {
                        Bitmap screen = (Bitmap)TaskHelpers.GetScreenshot(safeTaskSettings).CaptureFullscreen();
                        Bitmap annotated = SmartCaptureManager.ApplySmartAnnotation(screen);
                        screen.Dispose();
                        UploadManager.RunImageTask(annotated, safeTaskSettings);
                    }
                    break;

                case HotkeyType.AllMonitorsCapture:
                    {
                        Bitmap allMonitors = SmartCaptureManager.CaptureAllMonitors();
                        UploadManager.RunImageTask(allMonitors, safeTaskSettings);
                    }
                    break;

                case HotkeyType.StartCaptureOnChange:
                    SmartCaptureManager.StartCaptureOnChange(
                        Program.Settings.CaptureOnChangeThresholdPercent > 0
                            ? Program.Settings.CaptureOnChangeThresholdPercent
                            : 5f,
                        safeTaskSettings);
                    break;

                case HotkeyType.StopCaptureOnChange:
                    SmartCaptureManager.StopCaptureOnChange();
                    break;
            }
        }
        ...
        public static Image FindMenuIcon<T>(T value) where T : Enum
        {
            ...
            else if (value is HotkeyType hotkeyType)
            {
                switch (hotkeyType)
                {
                    ...
                    case HotkeyType.SmartSubjectCrop: return Resources.monitor;
                    case HotkeyType.ScreenDiff: return Resources.monitor;
                    case HotkeyType.ClipboardHistory: return Resources.clipboard_block;
                    case HotkeyType.StartTimelapse: return Resources.camcorder_pencil;
                    case HotkeyType.StopTimelapse: return Resources.camcorder_pencil;
                    case HotkeyType.CapturePrivacyBlur: return Resources.image_saturation;
                    case HotkeyType.SmartAnnotate: return Resources.edit_drop_cap;
                    case HotkeyType.AllMonitorsCapture: return Resources.monitor;
                    case HotkeyType.StartCaptureOnChange: return Resources.camcorder_pencil;
                    case HotkeyType.StopCaptureOnChange: return Resources.camcorder_pencil;
                    // Other
