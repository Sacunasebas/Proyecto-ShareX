...
namespace ShareX
{
    public partial class HotkeySelectionControl : UserControl
    {
        ...
        public HotkeySelectionControl(HotkeySettings hotkeySettings)
        {
            ...
            InitializeComponent();
            UpdateControls();
            cmsTask.AutoSize = false;
            cmsTask.Height = 600;
            ToolStripManager.Renderer = new ToolStripProfessionalRenderer();
            cmsTask.LayoutStyle = ToolStripLayoutStyle.Flow;
            ...
            // Smart Tools submenú
            ToolStripMenuItem tsmiSmart = new ToolStripMenuItem("⚡ Smart Tools");
            tsmiSmart.Image = TaskHelpers.FindMenuIcon(HotkeyType.SmartSubjectCrop);
            HotkeyType[] smartTypes = {
    HotkeyType.SmartSubjectCrop, HotkeyType.ScreenDiff,
    HotkeyType.ClipboardHistory, HotkeyType.StartTimelapse,
    HotkeyType.StopTimelapse, HotkeyType.CapturePrivacyBlur,
    HotkeyType.SmartAnnotate, HotkeyType.AllMonitorsCapture,
    HotkeyType.StartCaptureOnChange, HotkeyType.StopCaptureOnChange
};
            foreach (HotkeyType ht in smartTypes)
            {
                ToolStripMenuItem item = cmsTask.Items
                    .OfType<ToolStripMenuItem>()
                    .FirstOrDefault(x => x.Tag is EnumInfo ei && ei.Value.Equals(ht));
                if (item != null)
                {
                    cmsTask.Items.Remove(item);
                    tsmiSmart.DropDownItems.Add(item);
                }
            }
            cmsTask.Items.Add(tsmiSmart);
