...
namespace ShareX
{
    public partial class MainForm : HotkeyForm
    {
        ...
        private void AfterShownJobs()
        {
            ...
            ToolStripMenuItem tsmiSmartTools = new ToolStripMenuItem("⚡ Smart Tools");
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("Smart Subject Crop", null, async (s, ev) => await TaskHelpers.ExecuteJob(HotkeyType.SmartSubjectCrop)));
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("Screen Diff", null, async (s, ev) => await TaskHelpers.ExecuteJob(HotkeyType.ScreenDiff)));
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("Clipboard History", null, async (s, ev) => await TaskHelpers.ExecuteJob(HotkeyType.ClipboardHistory)));
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("Start Timelapse", null, async (s, ev) => await TaskHelpers.ExecuteJob(HotkeyType.StartTimelapse)));
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("Privacy Blur", null, async (s, ev) => await TaskHelpers.ExecuteJob(HotkeyType.CapturePrivacyBlur)));
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("Smart Annotate", null, async (s, ev) => await TaskHelpers.ExecuteJob(HotkeyType.SmartAnnotate)));
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("All Monitors Capture", null, async (s, ev) => await TaskHelpers.ExecuteJob(HotkeyType.AllMonitorsCapture)));
            tsddbTools.DropDownItems.Add(tsmiSmartTools);
        }
...
#pragma warning restore WFO5001
        ...
        {
            AfterShownJobs();
            // Monitoreo del portapapeles para historial
            if (Program.Settings.EnableClipboardMonitor)
            {
                System.Windows.Forms.Timer clipTimer = new System.Windows.Forms.Timer();
                clipTimer.Interval = 1000;
                clipTimer.Tick += (s, e) => SmartCaptureManager.PollClipboard();
                clipTimer.Start();
            }
        }
        ...
        private void tsmiSmartTools_Click(object sender, EventArgs e)
        {
            ToolStripMenuItem tsmiSmartTools = new ToolStripMenuItem("⚡ Smart Tools");
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("Smart Subject Crop", null, (s, ev) => TaskHelpers.ExecuteJob(HotkeyType.SmartSubjectCrop)));
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("Screen Diff", null, (s, ev) => TaskHelpers.ExecuteJob(HotkeyType.ScreenDiff)));
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("Clipboard History", null, (s, ev) => TaskHelpers.ExecuteJob(HotkeyType.ClipboardHistory)));
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("Start Timelapse", null, (s, ev) => TaskHelpers.ExecuteJob(HotkeyType.StartTimelapse)));
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("Privacy Blur", null, (s, ev) => TaskHelpers.ExecuteJob(HotkeyType.CapturePrivacyBlur)));
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("Smart Annotate", null, (s, ev) => TaskHelpers.ExecuteJob(HotkeyType.SmartAnnotate)));
            tsmiSmartTools.DropDownItems.Add(new ToolStripMenuItem("All Monitors Capture", null, (s, ev) => TaskHelpers.ExecuteJob(HotkeyType.AllMonitorsCapture)));
            tsmiMonitorTest.Owner?.Items.Add(tsmiSmartTools);
        }
